...
void solve(char *filename, int race_time) {
  ...
  long best = -1, distances[idx];
  for (int j = 0; j < idx; j++)
    distances[j] = 0;

  for (int i = 0; i < race_time; i++) {
    for (int j = 0; j < idx; j++) {
      distances[j] = distance_after(&deer[j], i + 1);
      best = distances[j] > best ? distances[j] : best;
    }
    for (int j = 0; j < idx; j++)
      if (distances[j] == best)
        deer[j].points++;
  ...
  best = -1;
  for (int j = 0; j < idx; j++)
    best = deer[j].points > best ? deer[j].points : best;

  printf("Winning points is: %ld\n", best);
  fclose(file);
}
...
// 1059 is the right answer for part 2
