...
uint make_valid_groups(uint num_presents, uint *inputs, uint *outputs) {
  uint target = sum_array(num_presents, inputs) / 4;
...
int solve(char *filename) {
  ...
  for (i = 0; i < num_least_entanglement; i++) {
    ...
    for (j = 0; j < num_orth1; j++) {
      num_orth2 = extract_orthogonal(ortho_groups1[j], ortho_groups1, num_orth1,
                                     ortho_groups2);
      for (k = 0; k < num_orth2; k++) {
        num_orth3 = extract_orthogonal(ortho_groups2[k], ortho_groups2,
                                       num_orth2, ortho_groups3);
        if (0 < num_orth3) {
  ...
  printf("Solution not at lowest entropy!!!");
  return EXIT_FAILURE;
}
...
// 74850409 is the correct answer
// Execution time was 7s
