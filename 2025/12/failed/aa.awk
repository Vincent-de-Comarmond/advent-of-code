...
/x/ {
	split("", packings, FS)
	determine_packings(packings, 8, 8)
	for (i = 1; i <= length(packings); i++) {
		print_matrix(packings[i])
	}
	exit (0)
	gsub(/:/, "", $0)
	gsub(/x/, " ", $0)
...
function is_packable_dfs(canvas, desired_presents, i, j, d, s, edges, undo, oldv, a, b, c, branches, branch_num)
{
	if (++calls % 100000 == 1) {
	...
	for (i = 1; i <= asorti(branch_num, a, "@val_num_asc"); i++) {
		for (j in branches[a[i]]) {
			split(a[i], b, SUBSEP)
			split(j, c, SUBSEP)
			oldv = desired_presents[c[1]]
			desired_presents[c[1]]--
			# --- Apply move in-place ---
			split("", undo, FS)
			insert_shape_inplace(canvas, b[1], b[2], GALLERY[c[1]][c[2]], undo)
			# --- Recurse depth-first ---
